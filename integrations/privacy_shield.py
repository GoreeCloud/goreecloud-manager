...
def _validate(payload: Any) -> PrivacyShieldSnapshot:
    ...
    if production_approved:
        ...
        detail = (
            f"{product} reports {active_count} active and {pending_count} pending-acceptance "
            "Privacy Shield capabilities; production approval remains false."
        )
...
def privacy_shield_snapshot() -> PrivacyShieldSnapshot:
    ...
    if path is None:
        return _unavailable(
            "Privacy Shield status is disabled until PRIVACY_SHIELD_STATUS_FILE points to an approved sanitized status document."
        )
    if not path.is_file():
