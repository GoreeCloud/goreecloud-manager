...
def synthetic_build_metadata(
    *,
    ...
    manifest_digest: str = "sha256:" + ("c" * 64),
) -> dict:
    return {
...
class OciBuildIdentityTests(TestCase):
    def test_build_metadata_requires_loaded_image_config_digest_match(self):
        ...
        self.assertEqual(parsed["manifest_digest"], "sha256:" + ("c" * 64))
    ...
    def test_build_metadata_rejects_non_manifest_descriptor(self):
        metadata = synthetic_build_metadata()
        metadata["containerimage.descriptor"]["mediaType"] = (
            "application/vnd.oci.image.index.v1+json"
        )
        with self.assertRaisesRegex(ValueError, "manifest type"):
            identity.validate_build_metadata(
                metadata,
                expected_image_id="sha256:" + ("b" * 64),
            )

    def test_identity_binds_source_image_and_manifest_without_publication_claim(self):
        revision = "a" * 40
        ...
        self.assertEqual(
            payload["image"]["build_output"]["manifest_digest"],
            "sha256:" + ("c" * 64),
        )
        self.assertTrue(payload["claims"]["loaded_image_matches_buildx_config_digest"])
        self.assertTrue(payload["claims"]["oci_manifest_digest_recorded"])
    ...
    def test_verify_fails_when_manifest_digest_changes(self):
        revision = "a" * 40
        image_reference = f"goreecloud-manager-security:{revision}"
        payload = identity.build_identity(
            inspection=synthetic_inspection(revision, image_reference),
    ...
    def test_json_output_is_stable_and_machine_readable(self):
        ...
        self.assertEqual(
            decoded["image"]["build_output"]["manifest_digest"],
            "sha256:" + ("c" * 64),
        )
