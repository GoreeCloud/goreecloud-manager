#!/usr/bin/env python3
"""Validate the GoreeCloud Manager production-readiness evidence inventory."""
...
DEFAULT_MANIFEST = Path(__file__).with_name("manager_production_readiness_manifest.json")
...
def validate_source_gates(
    manifest: dict[str, Any], repo_root: Path, check_repository: bool
) -> int:
    ...
    for index, gate in enumerate(gates):
        ...
        if (
            ...
            or not jobs
            or not all(isinstance(job, str) and job for job in jobs)
        ):
        ...
        if check_repository:
            ...
            if actual_name != name:
                fail(
                    f"{path}: manifest name {name!r} does not match workflow name {actual_name!r}"
                )
    ...
    if check_repository:
        ...
        if actual_paths != manifested_paths:
            ...
            fail(
                f"workflow inventory drift; unmanifested={missing!r}, "
                f"missing_on_disk={stale!r}"
            )
    ...
    if declared != total_jobs:
        fail(
            f"declared_effective_check_count={declared} but manifested jobs total {total_jobs}"
        )
    return total_jobs
...
def validate_production_state(manifest: dict[str, Any], outstanding: int) -> None:
    ...
    if status == "not-approved":
        ...
        if outstanding == 0:
            fail(
                "not-approved state with zero outstanding target evidence requires explicit review"
            )
        return
...
def validate_manifest(
    ...
    if manifest["service"] != "GoreeCloud Manager":
        fail("service must equal 'GoreeCloud Manager'")
...
def self_test(manifest: dict[str, Any]) -> None:
    ...
    duplicate["target_environment_evidence"].append(
        copy.deepcopy(duplicate["target_environment_evidence"][0])
    )
    ...
    expect_invalid(
        "satisfied-without-evidence", incomplete, "is required when evidence is satisfied"
    )
    ...
    approved_early["production_state"] = {
        ...
        "approved_at": "2026-08-14T00:00:00-05:00",
    }
    expect_invalid(
        "approved-with-outstanding-evidence", approved_early, "remains outstanding"
    )

    stale_approval = copy.deepcopy(manifest)
    stale_approval["production_state"]["approval_reference"] = "synthetic stale reference"
    expect_invalid(
        "not-approved-with-approval-metadata",
        stale_approval,
        "must not contain approval metadata",
    )

    secret = copy.deepcopy(manifest)
    secret["source_evidence_gates"][0]["evidence_scope"] = (
        "Authorization: Bearer abcdefghijklmnopqrstuvwxyz123456"
    )
    expect_invalid("sensitive-value", secret, "possible reusable sensitive value")

    satisfied = copy.deepcopy(manifest)
    item = satisfied["target_environment_evidence"][0]
    item.update(
        {
            "status": "satisfied",
            "evidence_reference": "synthetic evidence record",
            "verified_at": "2026-08-14T00:15:00-05:00",
            "verified_by": "synthetic reviewer",
        }
    )
    result = validate_manifest(satisfied, check_repository=False)
    if result["target_satisfied"] != 1:
        raise AssertionError("complete synthetic satisfied evidence did not validate")
    print("PASS complete synthetic satisfied target evidence")
...
def main() -> int:
    ...
    try:
        ...
        print(
            "Manager production readiness manifest valid: "
